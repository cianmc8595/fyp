...
$studentID = $username = $password = $confirm_password = $email = $firstname = $surname = $yearInSchool = $school = "";
$studentID_err = $username_err = $password_err = $confirm_password_err = $email_err = $firstname_err = $surname_err = $yearInSchool_err = $school_err = "";
...
if(isset($_POST['register'])){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        if ($conn_found) {
            ...
            $sql->bind_param('sss', $StudentUsername, $StudentUsername, $StudentUsername);
            
            // Set parameters
            $StudentUsername = trim($_POST["username"]);
    ...
    if(empty(trim($_POST['yearInSchool']))){
        ...
        $yearInSchool = trim($_POST['yearInSchool']);
    }
    ...
    $idFetch = $conn_found->prepare('SELECT studentID FROM students ORDER BY studentID DESC LIMIT 1');
    
    $idFetch->execute();
    $idFetch->store_result();
    
    $idFetch->bind_result($studentID); 
    $idFetch->fetch();
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($firstname_err) && empty($surname_err) && empty($email_err) && empty($yearInSchool_err) && empty($school_err) && empty($studentID_err)){
        
        // Prepare an insert statement
        $SQL = $conn_found->prepare("INSERT INTO students (studentID, username, password, email, firstname, surname, yearInSchool, school, enabled) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
         
        if($conn_found){
            // Bind variables to the prepared statement as parameters
            $SQL->bind_param('sssssssss', $param_studentID, $param_username, $param_password, $param_email, $param_firstname, $param_surname, $param_yearInSchool, $param_school, $param_enabled);
            ...
            $param_email = $email;
            $param_yearInSchool = $yearInSchool;
            $param_school = $school;
            ...
            if($SQL->execute()){
                ...
                if($check !== false){
                    ...
                    $userIDforImage = $param_studentID;
                    /*
...
<!-- START of modified code -->
    ...
    <div class="section section-header">
        <div class="parallax filter filter-color-blue" style="height:1150px;">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" style="padding-bottom:85px; margin-top:-50px;
                    ...
                    <div class="form-group <?php echo (!empty($yearInSchool_err)) ? 'has-error' : ''; ?>">
                		<select required type="text" class="form-control" name="yearInSchool">
                    		<option selected value="">What year are you in?</option>
					...
					<div class="form-group <?php echo (!empty($school_err)) ? 'has-error' : ''; ?>">
                		<select type="text" class="form-control" name="school">
                    		<option selected value="">Select your school from the list</option><option value="school">My School is not on this list</option><option value="Ashton School">Ashton School</option>
