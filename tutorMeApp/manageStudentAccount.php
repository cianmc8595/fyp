...
/*$mysqli = new mysqli('127.0.0.1', 'cianmc85', '', 'project_db') */
  $mysqli = new mysqli('eu-cdbr-west-02.cleardb.net', 'bdff3cc89b8df5',
            ...
            or die ('Cannot connect to db');
                ...
                $dataRetrieval = mysqli_query($mysqli, "SELECT * FROM students where studentID = '".$_SESSION['studentID']."'");
                ...
                $yearInSchool = $dataRetrievalRow['yearInSchool'];
                $school = $dataRetrievalRow['school'];
     
     $link = mysqli_connect("eu-cdbr-west-02.cleardb.net", "bdff3cc89b8df5",
                        "25912b2f", "heroku_6a6bf0a23aababd");
...
if (isset($_POST['saveChanges'])) {
    ...
    if(empty(trim($_POST['yearInSchool']))){
        $yearInSchool_err = "Please enter the year you are in school.";     
    } else{
        $updatedYearInSchool = trim($_POST['yearInSchool']);
    }
    ...
    if(empty(trim($_POST['school']))){
        ...
        $updatedSchool = trim($_POST['school']);
    }
    
    // Check input errors before inserting in database
    if(empty($firstname_err) && empty($surname_err) && empty($email_err) && empty($yearInSchool_err) && empty($school_err)){
    
        if($link === false){
            die("ERROR: Could not connect. " . mysqli_connect_error());
        }

        $sql = "UPDATE students SET email=?, firstname=?, surname=?, yearInSchool=?, school=? WHERE studentID=?";

        if($stmt = mysqli_prepare($link, $sql)){
        
            mysqli_stmt_bind_param($stmt, "ssssss", $updatedEmail, $updatedFirstname, $updatedSurname, $updatedYearInSchool, $updatedSchool, $_SESSION['studentID']);
        
            if(mysqli_stmt_execute($stmt)){
                        

                echo "Record updated successfully";
                header("location: manageStudentAccount.php");
            } else {
...
$db = "heroku_6a6bf0a23aababd";
    ...
    $imageCheckSql = "SELECT image FROM images WHERE userID ='".$_SESSION['studentID']."' AND usertype = 'Student'";
    ...
    if ($imageCheckSqlResult->num_rows > 0) {
        ...
        $dbName     = 'heroku_6a6bf0a23aababd';
            ...
            //Insert image content into database
             $insert = $db->query("UPDATE images SET image='$imgContent' WHERE userID='".$_SESSION['studentID']."' AND usertype='Student'");
            if($insert){
    ...
    if($check !== false){
        $image = $_FILES['image']['tmp_name'];
        $imgContent = addslashes(file_get_contents($image));
        $usertypeForImage = "Student";
        $userIDforImage = $_SESSION['studentID'];
        /*
         * Insert image data into database
         */
...
<!-- START of modified code -->
    ...
    <div class="section section-header">
        <div class="parallax filter filter-color-blue" style="height:900px;">
            ...
            <div class="login-form">
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" style="margin-top:-50px; height:630px; padding-bottom:85px; width:60%; float: left; -webkit-border-radius: 20px 0px 0px 20px;-moz-border-radius: 20px 0px 0px 20px;border-radius: 20px 0px 0px 20px;">
		            <div class="avatar avatar-danger" style="margin-top:-35px;">
			            <img class="img-circle" src="assets/img/faces/studentsIcon.png" alt="">
		            </div>
                    <h2 class="text-center">Student Account Management</h2>   
                    ...
                    <div class="form-group <?php echo (!empty($yearInSchool_err)) ? 'has-error' : ''; ?>">
                        <label for="yearInSchool" class:"form-control" style="margin-left:70px;">Year In School</label>
                		<select required type="text" class="form-control" name="yearInSchool">
                    		<option selected value="<?php echo $yearInSchool; ?>"><?php echo $yearInSchool; ?></option>
                    		<option value="4th">4th</option>
                    		<option value="5th">5th</option>
                    		<option value="6th">6th</option>
                		</select>
                		<span class="help-block"><?php echo $yearInSchool_err; ?></span>
            		</div>
                    <div class="form-group <?php echo (!empty($school_err)) ? 'has-error' : ''; ?>">
                        <label for="school" class:"form-control" style="margin-left:70px;">School</label>
                		<select type="text" class="form-control" name="school">
                    		<option selected value="<?php echo $school; ?>"><?php echo $school; ?></option><option value="school">My School is not on this list</option><option value="Ashton School">Ashton School</option>
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" style="margin-top:-50px; height:630px; padding-bottom:85px; width:35%; float: right; -webkit-border-radius: 0px 20px 20px 0px;-moz-border-radius: 0px 20px 20px 0px;border-radius: 0px 20px 20px 0px;">
