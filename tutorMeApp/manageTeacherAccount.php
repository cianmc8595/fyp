...
//DB details
    ...
    $result = $db->query("SELECT image FROM images WHERE userID = '" .$_SESSION['teacherID']."' AND usertype = '" .$_SESSION['usertype']."'");
...
if (isset($_POST['saveChanges'])) {
    ...
    if(empty(trim($_POST['subject']))){
        ...
        $updatedSubject = trim($_POST['subject']);
    }
    ...
    if(empty($firstname_err) && empty($surname_err) && empty($email_err) && empty($subject_err) && empty($school_err)){
    
        if($link === false){
            die("ERROR: Could not connect. " . mysqli_connect_error());
        }

        $sql = "UPDATE teachers SET email=?, firstname=?, surname=?, subject=?, school=? WHERE teacherID=?";

        if($stmt = mysqli_prepare($link, $sql)){
        
            mysqli_stmt_bind_param($stmt, "ssssss", $updatedEmail, $updatedFirstname, $updatedSurname, $updatedSubject, $updatedSchool, $_SESSION['teacherID']);
        
            if(mysqli_stmt_execute($stmt)){
                        

                echo "Record updated successfully";
                header("location: manageTeacherAccount.php");
            } else {
...
$db = "heroku_6a6bf0a23aababd";
    ...
    $imageCheckSql = "SELECT image FROM images WHERE userID ='".$_SESSION['teacherID']."' AND usertype = 'Teacher'";
...
$check = getimagesize($_FILES["image"]["tmp_name"]);
    if($check !== false){
        ...
        $insert = $db->query("UPDATE images SET image='$imgContent' WHERE userID='".$_SESSION['teacherID']."' AND usertype='Teacher'");
        if($insert){
    ...
    if($check !== false){
        ...
        $userIDforImage = $_SESSION['teacherID'];
        /*
         * Insert image data into database
         */
            ...
            <div class="login-form">
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" style="margin-top:-50px; height:630px; padding-bottom:85px; width:60%; float: left; -webkit-border-radius: 20px 0px 0px 20px;-moz-border-radius: 20px 0px 0px 20px;border-radius: 20px 0px 0px 20px;">
		            <div class="avatar avatar-danger" style="margin-top:-35px;">
			            <img class="img-circle" src="assets/img/faces/teachersIcon.png" alt="">
		            </div>
                    <h2 class="text-center">Teacher Account Management</h2>   
                    ...
                    <div class="form-group <?php echo (!empty($subject_err)) ? 'has-error' : ''; ?>">
                        <label for="subject" class:"form-control" style="margin-left:70px;">Subject</label>
                		<select required type="text" class="form-control" name="subject">
